...
export default function EditDiscountCodePage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      fetch('/api/admin/registration-types').then((r) => r.json()),
      fetch(`/api/admin/discount-codes/${id}`).then((r) => r.json()),
    ]).then(([catRes, typeRes, codeRes]) => {
      if (catRes.success) setCategories(catRes.data)
      if (typeRes.success) setTypes(typeRes.data)
      if (codeRes.success) {
        const d: DiscountCode = codeRes.data
        setCode(d.code)
        setScope(d.scope)
        ...
        const cr: Record<number, { discount_type: 'percent' | 'fixed'; discount_value: string }> = {}
        const tr: Record<number, { discount_type: 'percent' | 'fixed'; discount_value: string }> = {}
        ;(d.rules || []).forEach((r: Rule) => {
          const row = {
            discount_type: (r.discount_type === 'fixed' ? 'fixed' : 'percent') as 'percent' | 'fixed',
            discount_value: String(r.discount_value),
          }
          if (r.category_id != null) cr[r.category_id] = row
          if (r.registration_type_id != null) tr[r.registration_type_id] = row
        })
        setCategoryRules(cr)
        setTypeRules(tr)
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const rules: Array<{ category_id?: number; registration_type_id?: number; discount_type: string; discount_value: string }> = []
    if (scope === 'category') {
      categories.forEach((cat) => {
        const r = categoryRules[cat.id]
        if (r && r.discount_value.trim() && !isNaN(Number(r.discount_value))) {
          rules.push({ category_id: cat.id, discount_type: r.discount_type, discount_value: r.discount_value.trim() })
        }
      })
    } else {
      types.forEach((t) => {
        ...
        if (r && r.discount_value.trim() && !isNaN(Number(r.discount_value))) {
          rules.push({ registration_type_id: t.id, discount_type: r.discount_type, discount_value: r.discount_value.trim() })
        }
      })
    }
    if (rules.length === 0) {
  ...
  const updateCategoryRule = (categoryId: number, field: 'discount_type' | 'discount_value', value: string) => {
    setCategoryRules((prev) => ({
      ...prev,
      [categoryId]: { ...prev[categoryId], discount_type: prev[categoryId]?.discount_type ?? 'percent', [field]: value },
    }))
  }
  const updateTypeRule = (typeId: number, field: 'discount_type' | 'discount_value', value: string) => {
    setTypeRules((prev) => ({
      ...prev,
      [typeId]: { ...prev[typeId], discount_type: prev[typeId]?.discount_type ?? 'percent', [field]: value },
    }))
  }
  ...
  return (
    <div className="p-6 max-w-4xl">
      <div className="mb-6">
        ...
        <h2 className="text-2xl font-bold text-gray-900 mt-2">İndirim kodu düzenle</h2>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6 bg-white rounded-lg border border-gray-200 p-6">
        ...
        <div>
          ...
          <div className="flex gap-6">
            ...
            <label className="flex items-center gap-2">
              <input type="radio" name="scope" checked={scope === 'type'} onChange={() => setScope('type')} />
              Kayıt türüne göre
            </label>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <input type="datetime-local" value={validFrom} onChange={(e) => setValidFrom(e.target.value)} className="w-full border border-gray-300 rounded-md px-3 py-2" />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Geçerlilik bitiş</label>
            <input type="datetime-local" value={validUntil} onChange={(e) => setValidUntil(e.target.value)} className="w-full border border-gray-300 rounded-md px-3 py-2" />
          </div>
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Kullanım limiti (boş = sınırsız)</label>
          <input type="number" min="1" value={usageLimit} onChange={(e) => setUsageLimit(e.target.value)} className="w-full max-w-xs border border-gray-300 rounded-md px-3 py-2" />
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Açıklama</label>
          <textarea value={description} onChange={(e) => setDescription(e.target.value)} className="w-full border border-gray-300 rounded-md px-3 py-2" rows={2} />
        </div>
              ...
              <table className="min-w-full">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th className="px-4 py-2 text-left text-xs font-medium text-gray-500">Değer</th>
                  </tr>
                </thead>
        ...
        <div className="flex gap-3">
          ...
          <Link href="/admin/discount-codes" className="px-4 py-2 border border-gray-300 rounded-lg hover:bg-gray-50">
            İptal
          </Link>
        </div>
      </form>
