  ...
  try {
    ...
    const [generalStats] = await pool.execute(`
      SELECT 
        ...
        SUM(CASE WHEN status = 1 THEN 1 ELSE 0 END) as active_registrations,
        SUM(CASE WHEN status = 0 THEN 1 ELSE 0 END) as cancelled_registrations
      FROM registrations
    `)
