...
export default function Step2Accommodation({ onNext, onBack }: Step2AccommodationProps) {
  ...
  const onSubmit = (data: AccommodationFormData) => {
    ...
    if (selectedRegistrationType) {
      ...
      console.log('💰 Step2 - Saving accommodation data:', accommodationData)
      updateAccommodation(accommodationData as any)
    } else {
      updateAccommodation(data)
    }
        ...
        {currencyType !== 'TRY' && (
          <div className="mb-4 bg-blue-50 border border-blue-200 rounded-lg p-3">
            <div className="flex items-center gap-2 text-sm text-blue-800">
              ...
              <span>
                {language === 'en' 
                  ? <>Prices are displayed in <strong>{currencyType}</strong>. Payment will be made in TRY.</>
                  : <>Fiyatlar <strong>{currencyType}</strong> cinsinden gösterilmektedir. Ödeme TL olarak yapılacaktır.</>
                }
              </span>
            </div>
          </div>
